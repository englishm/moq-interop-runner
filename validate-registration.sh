...
MOXYGEN_PROFILE="$SCRIPT_DIR/docs/moxygen-relay-support-profile.json"
...
#############################################################################
# 3. Optional conformance-profile validation (hard gate when present)
#############################################################################
profile_error=""
if [ -f "$MOXYGEN_PROFILE" ]; then
    profile_error="$(
        "$SCRIPT_DIR/scripts/validate-moxygen-schema.sh" &&
        "$SCRIPT_DIR/scripts/validate-moxygen-profile.sh" &&
        "$SCRIPT_DIR/scripts/test-moxygen-profile-validator.sh"
    2>&1)" || PROFILE_OK=false
fi
...
if [ -f "$MOXYGEN_PROFILE" ]; then
    if [ "$PROFILE_OK" = true ]; then
        md "**Conformance profile:** ✅ schema, profile, result, and mutation checks passed"
    else
        md "**Conformance profile:** ❌ validation failed"
        md ""
        md '```'
        md "${profile_error:-validation failed}"
        md '```'
    fi
    md ""
fi
...
# --- Footer / headline ---
md "---"
if [ "$SCHEMA_OK" = true ] && [ "$TARGET_DRAFT_OK" = true ] && [ "$PROFILE_OK" = true ]; then
...
[ "$SCHEMA_OK" = true ] && [ "$TARGET_DRAFT_OK" = true ] && [ "$PROFILE_OK" = true ]
