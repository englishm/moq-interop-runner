...
imquic, moq-rs-draft-18, moqx, and moxygen, then runs the xquic client against
the xquic relay as an unfiltered self-pair baseline. It retains one tcpdump
capture per external relay plus one per xquic relay case. The top-level
`report.html` is generated by the runner's standard `generate-report.sh`;
`case-summary.json` retains all 66 individual case results.
