...
moxygen and the controlled Rust driver are independent evidence bindings. Neither is a protocol oracle: the pinned target draft and gate semantic contract remain authoritative.
...
- The `independent_driver` contract uses independently authored publisher fixtures, subscriber assertions, namespaces, fixed payload constants, and planned functions. The current binding metadata identifies moq-rs as the first controlled Rust implementation; that implementation is non-normative and not privileged. It must not encode or call moxygen's tuple generator or verifier. Payload constants are unique per object; they are not generated from a template, and evidence must match their exact bytes.
...
For the controlled independent binding:
...
For either binding, apply the same gate-level semantic criteria to observations and use that binding's deterministic vector only to identify expected fixture instances. A successful independent result records setup, readiness at no more than 12,000 ms (10,000 ms peer rendezvous plus 2,000 ms client margin), and delivery/terminal observation at no more than 10,000 ms, for a 22,000 ms per-case timeout. A successful moxygen result records shared publisher readiness at no more than 10,000 ms and a case phase no longer than its selected 10,000 ms timeout. Non-pass outcomes may record only phases reached under the same ceilings. Aggregate `duration_ms` may include additional orchestration and is not capped by the selected timeout, but it must be at least the sum of all recorded wall-clock phases. Cross-stream reordering is permitted; ordering within a subgroup is not. Duplicate, missing, rewritten, or unexpected fixture objects fail closed.

## Common Evidence And Results

An evaluator `pass` requires exactly the selected gate's complete `required_assertion_ids` set, every assertion at pass with evidence references, all seven profile evidence categories, exact revisions, actual protocol metadata, and deployment provenance. A `fail` requires that same complete set and verified evidence/provenance, with at least one failed assertion. `inconclusive` requires a not-observed assertion or non-verified evidence/provenance; an all-pass, fully verified record is not inconclusive. `unsupported` requires both evaluator and claimed outcome to be unsupported, no contradiction classification, and assertions either absent or not-observed; it cannot carry a complete all-pass verified assertion set. Assertions from another gate or one arbitrary assertion cannot establish an outcome. The `publisher-readiness` and `delivery-observations` assertions are binding-specific. moxygen requires same-case REQUEST_OK plus wire subgroup and FIN/reset evidence. The current Rust binding requires SUBSCRIBE_OK, exact-track routing, serve-model observations, and TRACK_ENDED; it does not directly observe PUBLISH_NAMESPACE REQUEST_OK, raw Object ID deltas, FIRST_OBJECT, or wire FIN/reset. Results conform to `#/definitions/result_record` in [`moxygen-relay-support-profile.schema.json`](./moxygen-relay-support-profile.schema.json).
