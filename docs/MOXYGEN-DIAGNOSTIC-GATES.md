...
For either binding, apply the same gate-level semantic criteria to observations and use that binding's deterministic vector only to identify expected fixture instances. A successful independent result records setup, readiness at no more than 10,000 ms, and delivery/terminal observation at no more than 10,000 ms. A successful moxygen result records shared publisher readiness at no more than 10,000 ms and a case phase no longer than its selected 10,000 ms timeout. Non-pass outcomes may record only phases reached. Aggregate `duration_ms` may include additional orchestration and is not capped by the selected timeout, but it must be at least the sum of all recorded wall-clock phases. Cross-stream reordering is permitted; ordering within a subgroup is not. Duplicate, missing, rewritten, or unexpected fixture objects fail closed.
