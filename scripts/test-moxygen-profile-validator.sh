...
jq -n '
  ...
  };
  {
    ...
    reproduction: {
      command: ["moq-test-client", "--test", "subscribe-one-subgroup-per-group"],
...
expect_profile_failure "swapped binding gate timeouts" '.execution_policy.diagnostic_gate_timeouts_ms[] |= {moxygen_driver:.independent_moq_test_client,independent_moq_test_client:.moxygen_driver}'
expect_profile_failure "insufficient independent delivery margin" '.execution_policy.independent_moq_test_client_timeout_components_ms.delivery_and_terminal_margin = 9999 | .execution_policy.diagnostic_gate_timeouts_ms[].independent_moq_test_client = 19999'
...
expect_profile_failure "version-specific function name" '.gates[0].bindings.independent_moq_test_client.planned_function = ("test_" + "moqt" + "18_subscribe_group_basic")'
...
expect_profile_failure "shortened payload alias" '.gates[1].bindings.independent_moq_test_client.vector.payloads.items[0].ascii = "one-subgroup-s40-o4"'
expect_profile_failure "namespace differs from semantic ID" '.gates[0].bindings.independent_moq_test_client.vector.namespace_prefix = "moq-test/interop/other" | .gates[0].bindings.independent_moq_test_client.vector.namespace_rule = "moq-test/interop/other/{publisher_cid}"'
...
expect_schema_profile_success "fully described verified support" '.support_verification.state = "verified" | .support_verification.verified_at = "2026-08-31T12:00:00Z" | .support_verification.pinned_implementation_revision = {source_sha:"4444444444444444444444444444444444444444",image_digest:"sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"} | .support_verification.evidence_links = [{kind:"result-record",locator:"artifact:result-record.json",sha256:"dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd"},{kind:"evidence-artifact",locator:"https://example.invalid/evidence.json",sha256:"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"}]'
...
expect_schema_profile_failure "swapped binding gate timeouts" '.execution_policy.diagnostic_gate_timeouts_ms[] |= {moxygen_driver:.independent_moq_test_client,independent_moq_test_client:.moxygen_driver}'
expect_schema_profile_failure "insufficient independent delivery margin" '.execution_policy.independent_moq_test_client_timeout_components_ms.delivery_and_terminal_margin = 9999 | .execution_policy.diagnostic_gate_timeouts_ms[].independent_moq_test_client = 19999'
...
expect_schema_profile_failure "version-specific function name" '.gates[0].bindings.independent_moq_test_client.planned_function = ("test_" + "moqt" + "18_subscribe_group_basic")'
